...
export function useMicrophoneAnalysis(sessionStatus: SessionStatus): UseMicrophoneAnalysisReturn {
  ...
  const analyzeMicrophoneAudio = useCallback(() => {
    ...
    const historySize = 3; // Shorter history for more responsive visualization
    ...
    const smoothingFactor = 0.4; // More responsive than agent audio
    ...
    const enhancedVolume = Math.min(smoothedVolumeRef.current * 3.0, 1.0);
