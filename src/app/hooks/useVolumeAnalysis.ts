...
interface UseVolumeAnalysisParams {
  audioElement: HTMLAudioElement | null;
  isActive: boolean;
}

export function useVolumeAnalysis({ audioElement, isActive }: UseVolumeAnalysisParams) {
  ...
  const setupAudioAnalysis = useCallback(async () => {
    if (!audioElement || !audioElement.srcObject) {
      console.log('[useVolumeAnalysis] No audio element or srcObject available');
      return;
    }
    
    try {
      console.log('[useVolumeAnalysis] Setting up audio analysis...');
      ...
      // Start analysis loop
      analyzeAudio();
      console.log('[useVolumeAnalysis] Audio analysis setup complete');
    } catch (error) {
      console.error('[useVolumeAnalysis] Audio analysis setup failed:', error);
    }
  }, [audioElement]);
  ...
  }, [isActive, setupAudioAnalysis, cleanupAudioAnalysis]);
