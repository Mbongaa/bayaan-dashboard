...
import { useImprovedWebGLService } from './useImprovedWebGLService';
...
interface GalaxyRendererResources {
...
/**
 * Service-based Galaxy renderer hook
 * 
 * Uses WebGL Context Service instead of owning WebGL resources directly.
 * This ensures the Galaxy renderer can survive React re-renders and
 * component unmounts without losing its rendering state.
 */
export function useServicedGalaxyRenderer(
  contextId: string,
  config: GalaxyConfig
) {
  const webglService = useImprovedWebGLService(contextId, {
    alpha: config.transparent,
    premultipliedAlpha: false,
    antialias: true
  });

  const resourcesRef = useRef<GalaxyRendererResources>({
    renderer: null,
    program: null,
    mesh: null,
    animateId: null
  ...
  // Initialize OGL renderer when WebGL context is available
  useEffect(() => {
    if (!webglService.context || !webglService.isMounted) {
      return;
    }

    const { canvas } = webglService.context;

    // Clean up existing resources
    cleanupRenderer();

    // Create OGL renderer with the existing canvas
    ...
    console.log(`[useServicedGalaxyRenderer] Initialized renderer for context: ${contextId}`);

    return () => {
      window.removeEventListener('resize', handleResize);
      cleanupRenderer();
    };
  }, [webglService.context, webglService.isMounted, config.transparent]);
  ...
  // Update uniforms when config changes (without recreating renderer)
  useEffect(() => {
  ...
  }, [webglService.context?.mountedElement, config.mouseInteraction]);
  ...
  const cleanupRenderer = useCallback(() => {
    ...
    // Note: We don't clean up the WebGL context here since it's managed by the service
  ...
  return {
    containerRef: webglService.containerRef,
    isInitialized: !!(resourcesRef.current.renderer && webglService.isMounted),
    isContextLost: webglService.isLost,
